...
CloudCode Desktop/CLI, Claude Desktop/CLI, Claude Code, Cursor, Hermes,
...
- Documents Across Context as a standalone memory plugin for Codex, CloudCode
  Desktop/CLI, Claude Desktop/CLI, Claude Code, AAA, and any MCP-capable host.
...
- Adds host-plugin context pack and search surfaces that can be consumed by
  Codex, CloudCode Desktop/CLI, Claude Desktop, Claude CLI, AAA, or another
...
| CloudCode Desktop/CLI | Uses the managed `~/.across/bin/across-context` MCP entrypoint where the host supports user-level MCP registration. |
| Claude Desktop/CLI | Uses the managed `~/.across/bin/across-context` MCP entrypoint where the host supports user-level MCP registration. |
...
- 明确 Across Context 是独立的通用记忆插件，可被 Codex、CloudCode
  Desktop/CLI、Claude Desktop/CLI、Claude Code、AAA 以及任何 MCP-capable
  host 安装和使用。
...
- 新增通用 Agent 插件上下文包和搜索接口，Codex、CloudCode Desktop/CLI、
  Claude Desktop、Claude CLI、AAA 或其他 MCP host 都可以通过托管插件边界使用。
...
| CloudCode Desktop/CLI | 在宿主支持用户级 MCP 注册时，使用 `~/.across/bin/across-context` 作为托管 MCP 入口。 |
| Claude Desktop/CLI | 在宿主支持用户级 MCP 注册时，使用 `~/.across/bin/across-context` 作为托管 MCP 入口。 |
